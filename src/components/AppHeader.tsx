...
/** Kopfzeile der eingeloggten App: Logo-Chip + Benachrichtigungen (+ Admin). */
    ...
    <header className="flex items-center justify-between border-b border-line px-4 py-3">
      ...
      <div className="flex items-center gap-3">
        <Link href="/benachrichtigungen" aria-label="Benachrichtigungen" className="relative text-muted">
          <Bell size={19} />
          {unread > 0 && (
            <span className="absolute -right-1 -top-1 h-2.5 w-2.5 rounded-full border border-panel bg-rot" />
          )}
        </Link>
        {isAdmin && (
          <Link href="/admin" aria-label="Admin-Bereich" className="text-gold">
            <Shield size={19} />
          </Link>
        )}
      </div>
